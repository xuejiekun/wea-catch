...
from ..base import WeaCatch, DataManager
from ..base.filectrl import get_all_file
...
class Main(QtGui.QMainWindow, Ui_main):
    ...
    def update_data(self):
        ...
        print(self.config.debug_data_dir)
...
class DownloadTask(QtCore.QThread):
    ...
    def run(self):
        ...
        for i in range(24):
            code = catch.download_time(self.config.debug_data_dir, start.strftime('%Y-%m-%d %H:%M:%S'), overwrite=self.config.overwrite)
            file_name = start.strftime('%Y%m%d_%H00') + r'.html'

            if code == WeaCatch.EXPIRE:
...
class UpdateTask(QtCore.QThread):
    ...
    def run(self):
        db = DataManager(database=self.config.database_file)
        file_list = get_all_file(self.config.debug_data_dir)
        ...
        for file in file_list:
            ...
            db.update_from_file(file, self.config.reload)
            index += 1
